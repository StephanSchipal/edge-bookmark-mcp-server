...
class BookmarkExporter:
    ...
    def __init__(self):
        ...
        logger.info(f"📤 BookmarkExporter initialized (pandas: {PANDAS_AVAILABLE}, jinja2: {JINJA2_AVAILABLE})")
...
{% endfor %}
    ...
    async def export_bookmarks(
        ...
        logger.info(f"📤 Exporting {len(sorted_bookmarks)} bookmarks to {format} format...")
    ...
    def _apply_filters(
        ...
        logger.info(f"🔍 Filtered bookmarks: {len(bookmarks)} -> {len(filtered)}")
        return filtered
    ...
    def _sort_bookmarks(
        ...
        if options.sort_by in sort_key_map:
            try:
                ...
                logger.debug(f"📊 Sorted by {options.sort_by} ({'asc' if options.sort_ascending else 'desc'})")
                return sorted_bookmarks
            except Exception as e:
                logger.warning(f"⚠️ Sorting failed: {e}, using original order")
    ...
    async def _export_xlsx(
        ...
        if not PANDAS_AVAILABLE:
            logger.warning("⚠️ pandas not available, falling back to CSV")
